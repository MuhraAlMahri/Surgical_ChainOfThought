...
Provides structure hints (NOT step-by-step instructions) and lets model generate reasoning.
"""
...
def build_cot_prompt(
    ...
    CRITICAL: Do NOT write step-by-step instructions. Provide structure hints
    and let the model generate its own reasoning flow.
    ...
    hints = {
        "abnormality_detection": (
            "Analyze the surgical scene for abnormalities or instruments. "
            "Consider what visual features indicate normality or abnormality."
        ),
        "characteristics": (
            "Examine the specific properties of identified findings. "
            "Consider attributes such as color, location, size, type, or quantity."
        ),
        "treatment": (
            "Consider the clinical implications based on observations. "
            "Think about diagnosis, treatment recommendations, or clinical significance."
        )
    }
    ...
    # Get structure hint
    hint = hints.get(category, hints["abnormality_detection"])
    ...
    prompt += "Think through your reasoning, then provide your answer.\n\nReasoning:"
...
def _add_temporal_context(previous_frame_info: Dict) -> str:
    ...
    if "summary" in previous_frame_info:
        context_parts.append(f"Previous frame: {previous_frame_info['summary']}")
    elif "observations" in previous_frame_info:
    ...
    # Motion description
    if "motion" in previous_frame_info:
        motion = previous_frame_info["motion"]
        if motion:
            context_parts.append(f"Motion: {motion}")
    elif "motion_description" in previous_frame_info:
