#!/usr/bin/env python3
"""
Evaluation Script for Experiment 5: Sequential Chain-of-Thought
Uses a single model with sequential reasoning stages:
...
def generate_prediction(model, processor, device, image, prompt_text, max_new_tokens=20):
...
def evaluate_model(model, processor, device, test_data, image_base_path, max_samples=None):
    """Evaluate model on test data using sequential CoT."""
    ...
    for item in tqdm(test_data, desc="Evaluating"):
        ...
        try:
            # Stage 1: Initial reasoning (use instruction if available)
            prompt_stage1 = instruction if instruction else question
            pred1 = generate_prediction(model, processor, device, image, prompt_stage1, max_new_tokens=20)
            
            # Stage 2: Build on Stage 1
            prompt_stage2 = f"{instruction if instruction else question}\n\nBased on the initial observation: {pred1}\n\nWhat is your refined answer?"
            pred2 = generate_prediction(model, processor, device, image, prompt_stage2, max_new_tokens=20)
            
            # Stage 3: Final answer
            prompt_stage3 = f"{instruction if instruction else question}\n\nInitial observation: {pred1}\nRefined answer: {pred2}\n\nWhat is your final, most concise answer?"
            pred3 = generate_prediction(model, processor, device, image, prompt_stage3, max_new_tokens=20)
            
            # Use Stage 3 as final prediction
            prediction = pred3
        ...
        results['predictions'].append({
            ...
            'instruction': instruction,  # The instruction/prompt actually used
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Experiment 5: Sequential CoT")
    ...
    print("=" * 80)
    print("EXPERIMENT 5 EVALUATION: Sequential Chain-of-Thought")
    print("=" * 80)
