#!/bin/bash
#SBATCH --job-name=verify_instructions
#SBATCH --output=/l/users/muhra.almahri/Surgical_COT/logs/verification_%A.txt
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --mem=8G
...
echo "=========================================="
echo "Verifying Revised Instructions"
echo "=========================================="
...
cat > scripts/verify_revised.py << 'EOFPYTHON'
import json
...
# Run verification
python scripts/verify_revised.py
...
if [ $exit_code -eq 0 ]; then
    ...
    echo "Next: Review sample outputs and prepare for advisor"
else
    echo ""
    echo "⚠️  ISSUES FOUND! Check output above for details."
    echo "Do not send to advisor until issues are resolved."
fi
