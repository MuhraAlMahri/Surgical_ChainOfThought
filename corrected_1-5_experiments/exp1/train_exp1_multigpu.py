...
def main(cfg_path):
    ...
    if local_rank != -1:
        ...
        print(f"🖥️ Running in single-GPU mode")
    ...
    # Only enrich on rank 0 to avoid race conditions
    if local_rank <= 0:
        ...
        # Other ranks just update paths without enriching
        for split in ["train_jsonl", "val_jsonl"]:
            inp = base_dir / cfg["data"][split]
            outp = str(inp).replace(".jsonl", ".enriched.jsonl")
            cfg["data"][split] = outp
    
    # Wait for rank 0 to finish enrichment
    if local_rank != -1:
